...
\pagestyle{fancy}
\lhead{}
\chead{\textbf{StoneCutter Language Spec v.0.8}}  % -- classification
\rhead{}
\lfoot{} %-- format: TR YYYY-RRR-V.V; y = year; r = report; v = version
\cfoot{\textbf{StoneCutter Language Spec v.0.8}}  % -- classification
...
\begin{center}
\begin{tabular}{l r}
Date: & January 18, 2022 \\ % Date the experiment was performed
Revision: & 0.8 \\         % revision number
...
\begin{versionhistory}
  \vhEntry{0.1}{02.15.2019}{JLeidel}{Initial public release}
  \vhEntry{0.2}{02.21.2019}{JLeidel}{Adding syntax support for instruction to instruction format association}
  \vhEntry{0.3}{04.24.2019}{JLeidel}{Adding register attribute field with PC register attribute}
  \vhEntry{0.4}{05.21.2019}{JLeidel}{Updating format for SEXT and ZEXT intrinsics; fixing MERGE intrinsic; added BSEL/FENCE intrinsics}
  \vhEntry{0.5}{10.15.2019}{JLeidel}{Adding NOP instrinsic}
  \vhEntry{0.6}{03.23.2020}{JLeidel}{Adding pipeline syntax, instruction field widths and additional register attributes; Updating \texttt{for} loop syntax}
  \vhEntry{0.7}{02.03.2021}{JLeidel}{Adding vector and matrix syntax to register class variables and variable declarations}
  \vhEntry{0.8}{01.18.2022}{RKabrick}{...}
\end{versionhistory}
\clearpage
...
First, the stage mnemonic (synonymous with \texttt{INSTNAME} in Listing\ref{lis:instprototype}), must be suffixed with a monotonically increasingly number
...
Finally, the user can pass arguments to the stage prototype so long as they are either an \texttt{instformat} field, \texttt{regclass}, or a 
variable so long as is passed as an output signal from another VLIW stage prototype which properly specifies it as an output (see \ref{sec:VLIWPipelineBlocks}).

\vspace{0.125in}
\begin{lstlisting}[frame=single,style=base,caption={VLIW Stage Prototype Format},captionpos=b,label={lis:vliwprototype}]
def STAGENAME_#:VLIW( ARG1 ARG2 ARG3 ... ) {
}
\end{lstlisting}
...
\subsection{VLIW Pipeline Blocks}
\label{sec:VLIWPipelineBlocks}
In addition to providing support for RISC pipelines, StoneCutter also gives users 
the tools necessary to construct their own VLIW pipelines. 
A VLIW stage is defined with the \texttt{def} keyword used for instruction prototypes however 
the stage's mnemonic must be suffixed with an underscore \texttt{\_} followed by a number 
used for deriving its position inside the pipeline. 
There are no requirements for the number, $n$ so long as there exists at least one other VLIW stage 
whose mnemonic number is equal to $n\pm1$.
Users can define seperate stages which share the same positional identifier
as seen in Listing~\ref{lis:vliw-stagenum-syntax}
It is worth noting that while one can define their stages in any order, the VLIW pipeliner will 
sort them in increasing order.  

\begin{lstlisting}[frame=single,style=base,caption={VLIW Stage Mnemonic Syntax},captionpos=b,label={lis:vliw-stagenum-syntax}]

#-- STAGE_0 is implied: fetch+crack
STAGE_1:VLIW( ... ) {
  ...
}
...
Another key aspect of designing a VLIW architecture is defining how data flows between stages 
throughout the pipeline stages. 
Users can leverage the \texttt{IN} and \texttt{OUT} instrinsics to dictate how data will be 
passed to and from the VLIW prototype stages. 
A user can pass a \texttt{instformat field}, \texttt{regclass}, or \texttt{variable} as an output (\texttt{OUT})
signal to be received as an input (\texttt{IN}) from another stage. 

\vspace{0.125in}
\begin{lstlisting}[frame=single,style=base,caption={VLIW IN/OUT Syntax},captionpos=b,label={lis:vliw-in-out-syntax}]
...
regclass CLASS1( DATATYPE REG1, DATATYPE REG2, ... )
...
...
STAGE_1:VLIW() {
  ...
  u64 TmpREG1 = LOADELEM( REG1, 64 )
  OUT(TmpREG1)
}

STAGE_2:VLIW( TmpREG1 ) {
  IN(TmpREG1)
} 

\end{lstlisting}

The example shown in Listing~\ref{lis:vliw-in-out-syntax} illutrates how one would link an output from \texttt{STAGE\_0}
to \texttt{STAGE\_1}. 
A temporary variable is created inside of \texttt{STAGE\_1} and passed to an \texttt{OUT} intrinsic. 
The user must then define at least one another VLIW pipeline stage prototype that takes in this temporary
inside its argument list \underline{\textbf{and}} define an \texttt{IN} at the beginning of the stage prototype
to ensure the compiler detects it as a link between stages. 
It is important to note that a single argument to an \texttt{OUT} intrinsic can be treated as an input to 
any other VLIW stage whose position in the pipeline follows. 
...
\begin{table}[h]
\begin{center}
\caption{IN Intrinsic}
\vspace{0.125in}
\label{tab:INIntrinsic}
\begin{tabular}{|c|l|}
\hline
\textbf{Mnemonic} & \texttt{IN(ARG)}\\
\hline
\textbf{Description} & Defines input to a VLIW pipeline stage.\\
\hline
  \textbf{Arguments} & ARG defines the \texttt{instformat} field, \texttt{register}, or \texttt{variable} input for the encompassing stage.\\
\hline
\multirow{3}{*}{\textbf{Example}} & def STAGE\_1:VLIW( ARG1 )\{\\
                                  &   IN(ARG1)\\
                                                    & \}\\
\hline
\textbf{Return} & Nothing is returned\\
...
\begin{table}[h]
\begin{center}
\caption{OUT Intrinsic}
\vspace{0.125in}
\label{tab:OUTIntrinsic}
\begin{tabular}{|c|l|}
\hline
\textbf{Mnemonic} & \texttt{OUT(ARG)}\\
\hline
\textbf{Description} & Defines output from a VLIW pipeline stage.\\
\hline
  \textbf{Arguments} & ARG defines the \texttt{instformat} field, \texttt{register}, or \texttt{variable} to be a VLIW output from the encompassing stage.\\
\hline
\multirow{3}{*}{\textbf{Example}} & def STAGE\_1:VLIW( ... )\{\\
                                  &   u64 TmpARG1 = LOADELEM(ARG1, 64)\\
                                  &   OUT(TmpARG1)\\
                                                    & \}\\
\hline
\textbf{Return} & Nothing is returned\\
